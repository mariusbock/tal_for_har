...
def run_temporalmaxer(val_sbjs, cfg, ckpt_folder, ckpt_freq, resume, rng_generator, run):
    ...
    val_sens_data = None    
    for sbj in val_sbjs:
        sbj_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], sbj + '.csv'), index_col=False, low_memory=False).replace({"label": cfg['label_dict']}).fillna(0)
    ...
    print("Number of learnable parameters for TemporalMaxer: {}".format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
    # optimizer
    ...
    for epoch in range(start_epoch, start_epoch + max_epochs):
        ...
        for val_sbj in val_sbjs:
            ...
            sbj_segments = segments_df[segments_df['video_id'] == val_sbj]
            sbj_sens_data = pd.read_csv(os.path.join(cfg['dataset']['sens_folder'], val_sbj + '.csv'), index_col=False, low_memory=False).replace({"label": cfg['label_dict']}).fillna(0).to_numpy()
            sbj_preds, sbj_gt, _ = convert_segments_to_samples(sbj_segments, sbj_sens_data, cfg['dataset']['sampling_rate'])
            v_preds = np.append(v_preds, sbj_preds)
            v_gt = np.append(v_gt, sbj_gt)
