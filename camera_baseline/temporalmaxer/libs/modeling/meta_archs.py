...
class TemporalMaxerArchitecture(nn.Module):
    ...
    def forward(self, video_list):
        ...
        v_video_list = video_list
        v_points = points
        v_fpn_masks = fpn_masks
        v_out_cls_logits = out_cls_logits
        v_out_offsets = out_offsets
            
        # return loss during training
        # generate segment/lable List[N x 2] / List[N] with length = B
        # assert video_list[0]['segments'] is not None, "GT action labels does not exist"
        # assert video_list[0]['labels'] is not None, "GT action labels does not exist"
        if video_list[0]['segments'] is None or video_list[0]['labels'] is None:
            losses = {'cls_loss': torch.tensor([0.0]),
            ...
            losses = self.losses(
                ...
                cls_targets, reg_targets, weights
                )
        ...
        if self.training:
            ...
            results = self.inference(
                v_video_list, v_points, v_fpn_masks,
                v_out_cls_logits, v_out_offsets
            )
