# ------------------------------------------------------------------------
# Utils used for postprocessing inertial and camera predictions
# ------------------------------------------------------------------------
...
def majorityVoting(index, array, kernel=5):
    groupSizeFloor = kernel // 2
    arrayCount = len(array)

    if index <= groupSizeFloor:
        first = 0
        last = kernel
    elif index >= arrayCount - (kernel // 2):
        first = arrayCount - kernel
        last = arrayCount
    else:
...
def combine_predictions(inertial_predictions, camera_predictions, ground_truth):
    combined = []
    changing_bool = True
    for in_p, ca_p, gt_p in zip(inertial_predictions, camera_predictions, ground_truth):
        if in_p == gt_p:
            combined.append(in_p)
        elif ca_p == gt_p:
            combined.append(ca_p)
        else:
            if changing_bool:
                combined.append(in_p)
                changing_bool = False
...
def compute_misalignment_measures(gt, pred, classes, has_null=True):
    underfill_ratio, deletion_ratio, merge_ratio, fragmentation_ratio, insertion_ratio, overfill_ratio = np.zeros(classes.stop), np.zeros(classes.stop), np.zeros(classes.stop), np.zeros(classes.stop), np.zeros(classes.stop), np.zeros(classes.stop)
