# ------------------------------------------------------------------------
# Sensitivity analysis diagnosis by Alwassel et al.
# https://github.com/HumamAlwassel/DETAD
# ------------------------------------------------------------------------
import math
from detad_analysis.action_detector_diagnosis import ActionDetectorDiagnosis
...
def compute_average_mAP_N_for_characteristic(sensitivity_analysis, characteristic_name):
    ...
    for characteristic_value, this_characteristic_gt in gt_by_characteristic:
        ...
        try:
            ...
            average_mAP_n_by_characteristic_value[characteristic_value] = np.NaN
...
def plot_sensitivity_analysis(sensitivity_analysis, save_filename, 
    ...
    if base_average_mAP_N > 0:
        for char_idx, characteristic_name in enumerate(characteristic_names):
            this_sensitivity_analysis = sensitivity_analysis_df_by_characteristic_name.get_group(characteristic_name)
            x_values = [current_x_value,current_x_value]
            y_values = this_sensitivity_analysis['ratio-value'].values*100
            y_values = [np.nanmin(y_values)/base_average_mAP_N,np.nanmax(y_values)/base_average_mAP_N]
            this_min_y_value,this_max_y_value=np.nanmin(y_values),np.nanmax(y_values)
    ...
    if (math.isnan(min_y_value)) or (math.isinf(min_y_value)):
        min_y_value = 0
    if (math.isnan(max_y_value)) or (math.isinf(max_y_value)):
        max_y_value = 1
...
def sensitivity_analysis(ground_truth_filename, subset, prediction_filename, output_folder, is_thumos14):
...
if __name__ == '__main__':
    ...
    sensitivity_analysis(args.ground_truth_filename, args.subset, args.prediction_filename, args.output_folder, args.is_thumos14)